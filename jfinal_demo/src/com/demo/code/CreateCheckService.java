...
public class CreateCheckService {
    ...
    static CreateCheckService me = new CreateCheckService();
    
    /**
     * 创建验证码
     * @param session 
     */
    public String createCode(String createTypeFlag, HttpSession session){
    	//1.在内存中创建一张图片
        ...
        //7.将随机数写在session里面
        session.setAttribute("servercode", random);
        File out = null;
        String os = System.getProperty("os.name");
        if(os != null && os.startsWith("Windows")){
        	out = new File("D:/test/code.jpg");
        }else{
        	out = new File("/opt/code.jpg");
        }
        if(out.exists()){
        	out.delete();
        }
